...
module.exports = async () => {
	...
	socket.on("error", (err) => {
		messages.pushLine(
			`{red-fg}WebSocket error: ${err.message || err}{/red-fg}`,
		);
		screen.render();
	});
	...
	socket.on("close", (code, reason) => {
		...
		messages.pushLine(
			"{yellow-fg}If this happens immediately after sending a message, your connection may be closing on the server due to an error or invalid token.{/yellow-fg}",
		);
		screen.render();
	});
	...
	socket.on("message", (data) => {
		...
		messages.pushLine(`${parsed.username || "System"}: ${parsed.message}`);
